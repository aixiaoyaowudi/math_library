...
static constexpr ull miller_rabin_bases_ull[]
    = { 2, 325, 9375, 28178, 450775, 9780504, 1795265022 };
bool
miller_rabin_u32 (ui k)
{
  ...
  for (ui c : miller_rabin_bases_ui)
    {
      ...
      while (d & 1 ^ 1)
        {
          if ((d >>= 1), ((res = basic::fast_pow (c, d, k)) == k - 1))
            break;
          else if (res != 1)
            {
              return false;
...
ui
miller_rabin_next_prime_u32 (ui k)
{
  while (!miller_rabin_u32 (k))
    ++k;
  return k;
}
ull
miller_rabin_next_prime_u64 (ull k)
{
  while (!miller_rabin_u64 (k))
    ++k;
  return k;
}
}
