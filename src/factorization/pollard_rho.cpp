...
{
pollard_rho_random_engine pollard_rho_random_device (
    std::chrono::high_resolution_clock::now ().time_since_epoch ().count ());
static ull
pollard_rho_u64 (ull x)
{
  ull s = 0, t = 0, c = ((ull)pollard_rho_random_device ()) % (x - 1) + 1,
      j = 0, k = 1, tmp = 1;
...
static ui
pollard_rho_u32 (ui x)
{
  ui s = 0, t = 0, c = ((ui)pollard_rho_random_device ()) % (x - 1) + 1, j = 0,
     k = 1, tmp = 1;
  for (k = 1;; k <<= 1, s = t, tmp = 1)
    {
      for (j = 1; j <= k; ++j)
...
static void
_factorize_u64 (ull n, ui cnt, std::vector<ull> &pms)
{
  if (n < 2)
    return;
...
static void
_factorize_u32 (ui n, ui cnt, std::vector<ui> &pms)
{
  if (n < 2)
    return;
...
std::vector<std::pair<ull, ui> >
pollard_rho_factorize_u64 (ull k)
{
  std::vector<ull> pms;
  _factorize_u64 (k, 1, pms);
  std::sort (pms.begin (), pms.end ());
  std::vector<std::pair<ull, ui> > res;
...
std::vector<std::pair<ui, ui> >
pollard_rho_factorize_u32 (ui k)
{
  std::vector<ui> pms;
  _factorize_u32 (k, 1, pms);
  std::sort (pms.begin (), pms.end ());
