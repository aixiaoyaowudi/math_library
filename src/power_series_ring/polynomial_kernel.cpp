...
power_series_ring::polynomial_kernel::polynomial_kernel_ntt::release ()
{
  ...
  num.reset ();
  fn = fb = mx = 0;
  for (ui i = 0; i < tmp_size; ++i)
    tt[i].reset ();
}
ui
power_series_ring::polynomial_kernel::polynomial_kernel_ntt::_fastpow (ui a,
                                                                       ui b)
{
  ui ans = li.v (1), off = a;
  while (b)
    ...
    ui max_conv_size, ui P0, ui G0)
{
  ...
  ws0  = create_aligned_array<ui, 64> (fn + 32);
  ws1  = create_aligned_array<ui, 64> (fn + 32);
  num  = create_aligned_array<ui, 64> (fn + 32);
  for (ui i = 0; i < tmp_size; ++i)
    tt[i] = create_aligned_array<ui, 64> (fn + 32);
  _inv[0] = li.v (1);
  for (ui i = 2; i <= fn + 32; ++i)
    _inv[i - 1] = li.mul (li.v (P - P / i), _inv[(P % i) - 1]);
  for (ui i = 1; i <= fn + 32; ++i)
    num[i - 1] = li.v (i);
  ui j0 = _fastpow (li.v (G), (P - 1) / fn),
     j1 = _fastpow (_fastpow (li.v (G), (P - 2)), (P - 1) / fn);
  for (ui mid = (fn >> 1); mid >= 1;
       mid >>= 1, j0 = li.mul (j0, j0), j1 = li.mul (j1, j1))
    {
      ui w0 = li.v (1), w1 = li.v (1);
      for (ui i = 0; i < mid; ++i, w0 = li.mul (w0, j0), w1 = li.mul (w1, j1))
...
void
power_series_ring::polynomial_kernel::polynomial_kernel_ntt::dif (
    ui *restrict p, ui n)
{
#if defined(__AVX512F__) && defined(__AVX512DQ__)
  ui len          = (1 << n);
  ...
  if (len < 16)
    {
      ui t1, t2;
      for (ui l = len; l >= 2; l >>= 1)
        for (ui j = 0, mid = (l >> 1); j < len; j += l)
          {
            ui restrict *p1 = p + j, *p2 = p + j + mid, *ww = ws + mid;
            for (ui i = 0; i < mid; ++i, ++p1, ++p2, ++ww)
...
power_series_ring::polynomial_kernel::polynomial_kernel_ntt::dit (
    ui *restrict p, ui n, bool inverse_coef)
{
#if defined(__AVX512F__) && defined(__AVX512DQ__)
  ui len          = (1 << n);
...
power_series_ring::polynomial_kernel::polynomial_kernel_ntt::internal_mul (
    ui *restrict src1, ui *restrict src2, ui *restrict dst, ui m)
{
  dif (src1, m);
  dif (src2, m);
#if defined(__AVX__) && defined(__AVX2__)
  if ((1 << m) < 8)
    {
      for (ui i = 0; i < (1 << m); ++i)
        dst[i] = li.mul (src1[i], src2[i]);
    }
  else
    {
      __m256i restrict *p1 = (__m256i *)src1, *p2 = (__m256i *)src2,
                       *p3 = (__m256i *)dst;
      for (ui i = 0; i < (1 << m); i += 8, ++p1, ++p2, ++p3)
        *p3 = la.mul (*p1, *p2);
    }
#else
  for (ui i = 0; i < (1 << m); ++i)
...
void
power_series_ring::polynomial_kernel::polynomial_kernel_ntt::
    internal_transpose_mul (ui *restrict src1, ui *restrict src2,
    ...
    const power_series_ring::poly &a, const power_series_ring::poly &b)
{
  ...
  std::memcpy (tt[0].get (), &a[0], sizeof (ui) * la);
  std::memset (tt[0].get () + la, 0, sizeof (ui) * ((1 << m) - la));
  std::memcpy (tt[1].get (), &b[0], sizeof (ui) * lb);
  std::memset (tt[1].get () + lb, 0, sizeof (ui) * ((1 << m) - lb));
...
power_series_ring::polynomial_kernel::polynomial_kernel_ntt::internal_inv (
    ui *restrict src, ui *restrict dst, ui *restrict tmp, ui *restrict tmp2,
    ui len)
{ //10E(n) x^n->x^{2n}
  if (len == 1)
    {
  ...
  std::memcpy (tmp, src, sizeof (ui) * len);
  std::memcpy (tmp2, dst, sizeof (ui) * (len >> 1));
  ...
  std::memset (tmp, 0, sizeof (ui) * (len >> 1));
  dif (tmp, __builtin_ctz (len));
#if defined(__AVX__) && defined(__AVX2__)
  if (len <= 4)
    {
  ...
  if (len <= 8)
    {
      for (ui i = (len >> 1); i < len; ++i)
        dst[i] = li.neg (tmp[i]);
    }
  else
    {
      __m256i restrict *p1 = (__m256i *)(tmp + (len >> 1)),
                       *p2 = (__m256i *)(dst + (len >> 1));
      for (ui i = 0; i < (len >> 1); i += 8, ++p1, ++p2)
...
void
power_series_ring::polynomial_kernel::polynomial_kernel_ntt::dif_xni (
    ui *restrict arr, ui n)
{
#if defined(__AVX__) && defined(__AVX2__)
  ui *restrict ws = ws0.get ();
  ui len          = (1 << n);
  if (len <= 4)
    {
      for (ui i = 0; i < len; ++i)
        arr[i] = li.mul (arr[i], ws[(len << 1) + i]);
    }
  else
    {
      __m256i restrict *p1 = (__m256i *)arr,
                       *p2 = (__m256i *)(ws + (len << 1));
      for (ui i = 0; i < len; i += 8, ++p1, ++p2)
        *p1 = la.mul (*p1, *p2);
    }
...
void
power_series_ring::polynomial_kernel::polynomial_kernel_ntt::dit_xni (
    ui *restrict arr, ui n)
{
  dit (arr, n);
#if defined(__AVX__) && defined(__AVX2__)
...
void
power_series_ring::polynomial_kernel::polynomial_kernel_ntt::
    internal_inv_faster (ui *restrict src, ui *restrict dst, ui *restrict tmp,
...
{ //9E(n) x^n->x^{2n}
  ...
  std::memcpy (tmp, src, sizeof (ui) * (len >> 1));
  std::memcpy (tmp2, dst, sizeof (ui) * (len >> 1));
#if defined(__AVX__) && defined(__AVX2__)
  if (len <= 8)
    {
      ...
      for (ui i = 0; i < (len >> 1); ++i)
        tmp[i] = li.add (tmp[i], li.mul (mip, src[i + (len >> 1)]));
    }
  else
    {
      __m256i mip          = _mm256_set1_epi32 (ws0[3]);
      __m256i restrict *p1 = (__m256i *)(src + (len >> 1)),
                       *p2 = (__m256i *)tmp;
      for (ui i = 0; i < (len >> 1); i += 8, ++p1, ++p2)
  ...
  if (len <= 8)
    {
      for (ui i = 0; i < (len >> 1); ++i)
        tmp[i] = li.mul (li.mul (tmp2[i], tmp2[i]), tmp[i]);
    }
  else
    {
      __m256i restrict *p1 = (__m256i *)tmp2, *p2 = (__m256i *)tmp;
      for (ui i = 0; i < (len >> 1); i += 8, ++p1, ++p2)
        (*p2) = la.mul ((*p2), la.mul ((*p1), (*p1)));
    }
#else
  for (ui i = 0; i < (len >> 1); ++i)
    tmp[i] = li.mul (li.mul (tmp2[i], tmp2[i]), tmp[i]);
  ...
  std::memcpy (tmp2, src, sizeof (ui) * len);
  std::memcpy (tmp3, dst, sizeof (ui) * (len >> 1));
  std::memset (tmp3 + (len >> 1), 0, sizeof (ui) * (len >> 1));
  dif (tmp2, __builtin_ctz (len));
  ...
  if (len <= 8)
    {
      for (ui i = 0; i < len; ++i)
        tmp2[i] = li.mul (li.mul (tmp3[i], tmp3[i]), tmp2[i]);
    }
  else
    {
  ...
  if (len <= 8)
  ...
  else
    {
      __m256i restrict *p1
          = (__m256i *)tmp,
          *p2 = (__m256i *)(tmp2 + (len >> 1)), *p3 = (__m256i *)(tmp2),
          *p4 = (__m256i *)(dst + (len >> 1)), *p5 = (__m256i *)(dst);
      __m256i mip = _mm256_set1_epi32 (ws0[3]),
              iv2 = _mm256_set1_epi32 (_inv[1]);
      for (ui i = 0; i < (len >> 1); i += 8, ++p1, ++p2, ++p3, ++p4, ++p5)
    ...
    const power_series_ring::poly &src)
{
  ...
  std::memcpy (tt[0].get (), &src[0], sizeof (ui) * la);
  std::memset (tt[0].get () + la, 0, sizeof (ui) * ((1 << m) - la));
...
power_series_ring::polynomial_kernel::polynomial_kernel_ntt::internal_ln (
    ui *restrict src, ui *restrict dst, ui *restrict tmp1, ui *restrict tmp2,
    ui *restrict tmp3, ui len)
{
#if defined(__AVX__) && defined(__AVX2__)
  ui pos               = 1;
  __m256i restrict *pp = (__m256i *)tmp1, *iv = (__m256i *)num.get ();
  ui restrict *p1 = src + 1;
  ...
  for (ui i = 1; i < len; ++i, ++p1, ++p2, ++p3)
    *p2 = li.mul ((*p1), (*p3));
  tmp1[len - 1]    = li.v (0);
#endif
  internal_inv (src, dst, tmp2, tmp3, len);
  std::memset (dst + len, 0, sizeof (ui) * len);
  std::memset (tmp1 + len, 0, sizeof (ui) * len);
  internal_mul (tmp1, dst, tmp2, __builtin_ctz (len << 1));
#if defined(__AVX__) && defined(__AVX2__)
  ui ps                 = 1;
  __m256i restrict *pp0 = (__m256i *)tmp2, *iv0 = (__m256i *)_inv.get ();
  ui restrict *p10 = dst + 1;
  ...
  for (ui i = 1; i < len; ++i, ++p10, ++p20, ++p30)
    *p10 = li.mul ((*p20), (*p30));
#endif
}
void
power_series_ring::polynomial_kernel::polynomial_kernel_ntt::
    internal_ln_faster (ui *restrict src, ui *restrict dst, ui *restrict tmp1,
                        ui *restrict tmp2, ui *restrict tmp3,
                        ui *restrict tmp4, ui len)
{ //12E(n)
  if (len < 4)
    {
      internal_ln (src, dst, tmp1, tmp2, tmp3, len);
      return;
    }
#if defined(__AVX__) && defined(__AVX2__)
  ui pos               = 1;
  ...
  tmp1[len - 1] = li.v (0);
#endif
  internal_inv_faster (src, dst, tmp2, tmp3, tmp4,
                       (len >> 1)); //tmp4=F_{n/2}(g0 mod x^{n/4})
  std::memcpy (dst, dst + (len >> 2), sizeof (ui) * (len >> 2));
  std::memset (dst + (len >> 2), 0, sizeof (ui) * (len >> 2));
  dif (dst, __builtin_ctz (len >> 1));
  std::memcpy (tmp2, tmp1 + (len >> 2), sizeof (ui) * (len >> 2));
  std::memset (tmp2 + (len >> 2), 0, sizeof (ui) * (len >> 2));
  ...
  if (len <= 8)
    {
      for (ui i = 0; i < (len >> 1); ++i)
        tmp2[i] = li.add (li.mul (tmp2[i], tmp4[i]), li.mul (dst[i], tmp1[i])),
        tmp1[i] = li.mul (tmp1[i], tmp4[i]);
    }
  else
    {
      __m256i restrict *p1 = (__m256i *)tmp2, *p2 = (__m256i *)tmp4,
                       *p3 = (__m256i *)dst, *p4 = (__m256i *)tmp1;
      for (ui i = 0; i < (len >> 1); i += 8, ++p1, ++p2, ++p3, ++p4)
        (*p1) = la.add (la.mul ((*p1), (*p2)), la.mul ((*p3), (*p4))),
        (*p4) = la.mul ((*p2), (*p4));
    }
#else
  for (ui i = 0; i < (len >> 1); ++i)
  ...
  dit (tmp2, __builtin_ctz (len >> 1));
#if defined(__AVX__) && defined(__AVX2__)
  if (len <= 16)
    {
      for (ui i = 0; i < (len >> 2); ++i)
        tmp1[i + (len >> 2)] = li.add (tmp1[i + (len >> 2)], tmp2[i]);
    }
  else
    {
      __m256i restrict *p1 = (__m256i *)(tmp1 + (len >> 2)),
                       *p2 = (__m256i *)tmp2;
      for (ui i = 0; i < (len >> 2); i += 8, ++p1, ++p2)
        (*p1) = la.add ((*p1), (*p2));
    }
#else
  for (ui i = 0; i < (len >> 2); ++i)
  ...
  std::memset (tmp2 + (len >> 1), 0, sizeof (ui) * (len >> 1));
  dif (tmp2, __builtin_ctz (len));
  std::memcpy (tmp3, src, sizeof (ui) * len);
  dif (tmp3, __builtin_ctz (len));
#if defined(__AVX__) && defined(__AVX2__)
  if (len <= 4)
    {
      for (ui i = 0; i < len; ++i)
        tmp3[i] = li.mul (tmp3[i], tmp2[i]);
    }
  else
    {
      __m256i restrict *p1 = (__m256i *)(tmp3), *p2 = (__m256i *)tmp2;
  ...
  if (len <= 8)
    {
      for (ui i = 0; i < (len >> 1); ++i)
        tmp3[i + (len >> 1)]
            = li.sub (tmp1[i + (len >> 1)], tmp3[i + (len >> 1)]);
    }
  else
    {
      __m256i restrict *p1 = (__m256i *)(tmp3 + (len >> 1)),
                       *p2 = (__m256i *)(tmp1 + (len >> 1));
      for (ui i = 0; i < (len >> 1); i += 8, ++p1, ++p2)
        (*p1) = la.sub ((*p2), (*p1));
    }
#else
  for (ui i = 0; i < (len >> 1); ++i)
    tmp3[i + (len >> 1)] = li.sub (tmp1[i + (len >> 1)], tmp3[i + (len >> 1)]);
#endif
  std::memcpy (tmp3, tmp3 + (len >> 2) * 3, sizeof (ui) * (len >> 2));
  std::memset (tmp3 + (len >> 2), 0, sizeof (ui) * (len >> 2));
  std::memcpy (tmp2, tmp3 + (len >> 1), sizeof (ui) * (len >> 2));
  std::memset (tmp2 + (len >> 2), 0, sizeof (ui) * (len >> 2));
  ...
  if (len <= 8)
    {
      for (ui i = 0; i < (len >> 1); ++i)
        tmp3[i] = li.add (li.mul (tmp3[i], tmp4[i]), li.mul (dst[i], tmp2[i])),
        tmp2[i] = li.mul (tmp2[i], tmp4[i]);
    }
  else
    {
      __m256i restrict *p1 = (__m256i *)tmp3, *p2 = (__m256i *)tmp4,
                       *p3 = (__m256i *)dst, *p4 = (__m256i *)tmp2;
      __m256i tt;
      for (ui i = 0; i < (len >> 1); i += 8, ++p1, ++p2, ++p3, ++p4)
        (*p1) = la.add (la.mul ((*p1), (*p2)), la.mul ((*p3), (*p4))),
        (*p4) = la.mul ((*p2), (*p4));
  ...
  if (len <= 16)
    {
      for (ui i = 0; i < (len >> 2); ++i)
        tmp1[i + (len >> 2) * 3] = li.add (tmp2[i + (len >> 2)], tmp3[i]);
    }
  else
    {
      __m256i restrict *p1 = (__m256i *)(tmp1 + (len >> 2) * 3),
                       *p2 = (__m256i *)(tmp2 + (len >> 2)),
                       *p3 = (__m256i *)(tmp3);
      for (ui i = 0; i < (len >> 2); i += 8, ++p1, ++p2, ++p3)
        (*p1) = la.add ((*p2), (*p3));
    }
#else
  for (ui i = 0; i < (len >> 2); ++i)
...
#if defined(__AVX__) && defined(__AVX2__)
  ui ps                 = 1;
  __m256i restrict *pp0 = (__m256i *)tmp1, *iv0 = (__m256i *)_inv.get ();
  ui restrict *p10 = dst + 1;
  for (; ps + 8 <= len; ps += 8, p10 += 8, ++pp0, ++iv0)
    _mm256_storeu_si256 ((__m256i *)p10, la.mul (*pp0, *iv0));
  dst[0] = li.v (0);
  for (; ps < len; ++ps)
    dst[ps] = li.mul (_inv[ps - 1], tmp1[ps - 1]);
#else
  dst[0]           = li.v (0);
...
power_series_ring::polynomial_kernel::polynomial_kernel_ntt::internal_exp (
    ui *restrict src, ui *restrict dst, ui *restrict gn, ui *restrict gxni,
    ui *restrict h, ui *restrict tmp1, ui *restrict tmp2, ui *restrict tmp3,
    ui len, bool calc_h)
{
  if (len == 1)
    {
...
#endif
  std::memmove (tmp1 + 1, tmp1, sizeof (ui) * (len >> 1));
  tmp1[0] = tmp1[(len >> 1)];
  std::memset (tmp1 + (len >> 1), 0, sizeof (ui) * (len >> 1));
  dif (tmp1, __builtin_ctz (len));
  std::memcpy (tmp3, h, sizeof (ui) * (len >> 1));
  std::memset (tmp3 + (len >> 1), 0, sizeof (ui) * (len >> 1));
  dif (tmp3, __builtin_ctz (len));
#if defined(__AVX__) && defined(__AVX2__)
  if (len < 8)
  ...
  if (len <= 8)
    ...
    {
      __m256i restrict *p1 = (__m256i *)tmp1, *p3 = (__m256i *)(tmp2),
                       *p4 = (__m256i *)(src + (len >> 1));
      ui *restrict p2      = _inv.get () + (len >> 1) - 1;
      for (ui i = 0; i < (len >> 1); i += 8, ++p1, p2 += 8, ++p3, ++p4)
        (*p3) = la.sub ((*p4),
  ...
  if (len <= 16)
    ...
    {
      __m256i restrict *p1 = (__m256i *)dst,
                       *p2 = (__m256i *)(dst + (len >> 2)),
                       *p3 = (__m256i *)(tmp1),
                       *p4 = (__m256i *)(ws0.get () + (len >> 1)),
                       *p5 = (__m256i *)(ws0.get () + (len >> 2));
      __m256i mip          = _mm256_set1_epi32 (ws1[3]);
      for (ui i = 0; i < (len >> 2); i += 8, ++p1, ++p2, ++p3, ++p4, ++p5)
  ...
  if (!calc_h)
    return;
  std::memcpy (gxni, dst, sizeof (ui) * (len >> 1));
  std::memcpy (tmp2, h, sizeof (ui) * (len >> 1));
#if defined(__AVX__) && defined(__AVX2__)
  if (len <= 8)
    {
      ui mip = ws0[3];
    ...
    {
      __m256i mip          = _mm256_set1_epi32 (ws0[3]);
      __m256i restrict *p1 = (__m256i *)(dst + (len >> 1)),
                       *p2 = (__m256i *)gxni;
      for (ui i = 0; i < (len >> 1); i += 8, ++p1, ++p2)
        (*p2) = la.add ((*p2), la.mul ((*p1), mip));
    }
  ...
  if (len <= 8)
    ...
    {
      __m256i restrict *p1 = (__m256i *)tmp2,
                       *p2 = (__m256i *)(tmp3 + (len >> 1)),
                       *p3 = (__m256i *)(tmp3),
                       *p4 = (__m256i *)(h + (len >> 1)), *p5 = (__m256i *)(h);
      __m256i mip = _mm256_set1_epi32 (ws0[3]),
              iv2 = _mm256_set1_epi32 (_inv[1]);
      for (ui i = 0; i < (len >> 1); i += 8, ++p1, ++p2, ++p3, ++p4, ++p5)
        (*p4) = la.mul (la.sub (la.mul (la.sub (la.add ((*p1), (*p3)),
...
power_series_ring::polynomial_kernel::polynomial_kernel_ntt::test (ui T)
{
  std::mt19937 rnd (default_mod);
  std::uniform_int_distribution<ui> rng{ 0, default_mod - 1 };
  ui len = (fn >> 2);
  for (ui i = 0; i < len; ++i)
...
power_series_ring::polynomial_kernel::polynomial_kernel_ntt::
    internal_multipoint_eval_interpolation_calc_Q (
        std::vector<poly> &Q_storage, const poly &input_coef, ui l, ui r,
        ui id)
{
  if (l == r)
    {
...
{
  ui point_count = b.size (), poly_count = a.size (),
...
{
  ui la = a.size (), lb = b.size (), len = std::max (la, lb);
  poly ret (len);
