...
static ull
calc_bju (ull j, ull u)
{
  __uint128_t cur = 0, u2 = __uint128_t (u) * u;
  for (ui i = 31; ~i; --i)
    cur = cur * u2 + coefs[j][i];
  cur *= u;
...
ull
binomial::fast_binomial_mod_2_64::factorial_odd (ull k)
{
  ull ans = 1;
  while (k)
    {
...
ull
binomial::fast_binomial_mod_2_64::factorial (ull k)
{
  ull cnt2 = 0;
  while (k)
    {
...
ull
binomial::fast_binomial_mod_2_64::binomial (ull upper, ull lower)
{
  if (upper < lower)
    return 0;
  ui cnt2 = __builtin_popcountll (lower) + __builtin_popcountll (upper - lower)
            - __builtin_popcountll (upper);
  if (cnt2 >= 64)
    return 0;
  ull c
      = basic::fast_pow (factorial_odd (lower) * factorial_odd (upper - lower),
                         (1ull << 63) - 1)
        * factorial_odd (upper);
  return c << cnt2;
}
