...
namespace math
{
	namespace basic
	{
		...
		typename std::enable_if<traits::func_call_valid_v<F,bool,std::remove_cv_t<std::remove_reference_t<B>>,Args...> &&
		std::is_integral_v<std::remove_cv_t<std::remove_reference_t<B>>>,
		...
		binary_search_min(B l0,B r0,F &&func,Args&&... args){
			...
			while(l<=r){
				mid=(l+r)/2;
				if(func(mid,args...)) ans=mid,r=mid-1;
				else l=mid+1;
			}
			return ans;
		}
		...
		binary_search_max(B l0,B r0,F &&func,Args&&... args){
			...
			while(l<=r){
				mid=(l+r)/2;
				if(func(mid,args...)) ans=mid,l=mid+1;
				else r=mid-1;
			}
			return ans;
		}
		template<typename B,typename F,typename... Args>
		typename std::enable_if<traits::func_call_valid_v<F,bool,std::remove_cv_t<std::remove_reference_t<B>>,Args...> &&
		std::is_floating_point_v<std::remove_cv_t<std::remove_reference_t<B>>>,
		std::remove_cv_t<std::remove_reference_t<B>>>::type
		binary_search_min(B l0,B r0,F &&func,Args&&... args){
			typedef typename std::remove_cv_t<std::remove_reference_t<B>> BB;
			...
			while(r-l>=eps){
				mid=(l+r)/2;
				if(func(mid,args...)) ans=mid,r=mid;
				else l=mid;
			}
			return ans;
		}
		...
		binary_search_max(B l0,B r0,F &&func,Args&&... args){
			...
			while(r-l>=eps){
				mid=(l+r)/2;
				if(func(mid,args...)) ans=mid,l=mid;
				else r=mid;
			}
			return ans;
		}
		template<typename B,typename F,typename... Args>
		typename std::enable_if<(!traits::func_call_valid_v<B,bool,std::remove_cv_t<std::remove_reference_t<B>>,F,Args...>) &&
		traits::func_call_valid_v<F,bool,std::remove_cv_t<std::remove_reference_t<B>>,Args...> &&
		std::is_floating_point_v<std::remove_cv_t<std::remove_reference_t<B>>>,
		std::remove_cv_t<std::remove_reference_t<B>>>::type
