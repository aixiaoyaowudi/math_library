...
namespace math
{
template <typename T, size_t align_val> struct aligned_delete
{
  void
...
template <typename T, size_t align_val>
aligned_array<T, align_val>
create_aligned_array (size_t size)
{
  return aligned_array<T, align_val> (new (std::align_val_t (align_val))
                                          T[size]);
}
namespace traits
{
...
struct op_valid_helper
{
  ...
  static auto test (int) -> std::is_same<
      std::remove_cv_t<std::remove_reference_t<decltype (std::declval<U> () (
...
template <typename X, typename Y, typename W, typename Op>
using op_valid = typename op_valid_helper<X, Y, W, Op>::type;
template <typename X, typename Y, typename W, typename Op>
constexpr bool op_valid_v = op_valid<X, Y, W, Op>::value;
template <typename X, typename Y, typename T, typename Op>
struct const_op_valid_helper
{
  ...
  static auto test (int) -> std::is_same<
      std::remove_cv_t<std::remove_reference_t<decltype (std::declval<U> () (
          std::declval<std::add_lvalue_reference_t<std::add_const_t<
...
template <typename X, typename Y, typename W, typename Op>
using const_op_valid = typename const_op_valid_helper<X, Y, W, Op>::type;
template <typename X, typename Y, typename W, typename Op>
constexpr bool const_op_valid_v = const_op_valid<X, Y, W, Op>::value;
template <typename F, typename R, typename... A>
using func_call_valid = typename std::is_invocable_r<R, F, A...>::type;
template <typename F, typename R, typename... A>
constexpr bool func_call_valid_v = func_call_valid<F, R, A...>::value;
...
template <typename T> struct has_unit_value_helper
{
  template <typename G>
  static auto test (int) -> typename std::enable_if<
      (std::is_arithmetic_v<G> || is_mod_int_v<G> || is_nimber_int_v<G>),
      std::true_type>::type;
  template <typename G> static auto test (...) -> std::false_type;
  using type = decltype (test<T> (0));
};
template <typename T>
using has_unit_value = typename has_unit_value_helper<T>::type;
template <typename T>
constexpr bool has_unit_value_v = has_unit_value<T>::value;
...
template <typename T,
          typename std::enable_if<is_mod_int<T>::value>::type * = nullptr>
T
unit_value ()
{
  return T (1);
...
template <typename T,
          typename std::enable_if<is_nimber_int<T>::value>::type * = nullptr>
T
unit_value ()
{
  return T (1);
...
template <typename T, typename std::enable_if<std::is_integral<T>::value>::type
                          * = nullptr>
struct double_width_int : size_int<2 * sizeof (T) * 8>
{
};
template <typename T>
using double_width_int_t = typename double_width_int<T>::type;
template <typename T, typename std::enable_if<std::is_integral<T>::value>::type
                          * = nullptr>
struct double_width_uint : size_uint<2 * sizeof (T) * 8>
{
};
template <typename T>
using double_width_uint_t = typename double_width_uint<T>::type;
}
}
}
